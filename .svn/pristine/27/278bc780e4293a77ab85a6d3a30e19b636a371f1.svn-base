...
function flying_images_add_responsiveness($images) {
    ...
    foreach ($images as $image) {
        ...
        if($max_width) {
			$available_widths = array_filter($available_widths, function ($width) use ($max_width) {
        ...
        // Generate sizes
        $largest_width = end($available_widths);
        $sizes = "(max-width: {$largest_width}px) 100vw, {$largest_width}px";
        $image->setAttribute("sizes", $sizes);

		// Add Max width to src
        if($max_width) $image->src = flying_images_add_query_string($image->src, "w", $max_width);
    }
}
