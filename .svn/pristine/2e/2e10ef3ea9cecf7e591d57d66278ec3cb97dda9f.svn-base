...
function flying_images_add_cdn_to_styles($styles, $compression_enabled, $quality) {
    ...
    foreach ($styles as $style) {
        ...
        if(preg_match($regex, $style->innertext, $matches)) {
            ...
            // Update style
            $style->innertext = "{$matches[1]}background:url({$image_url}){$matches[3]}";
        }
    }
}
    ...
    foreach ($images as $image) {
        ...
        if(preg_match($regex, $image->style, $matches)) {
            ...
            if($lazy_loading_enabled) {
                ...
                $style = "background:none;{$matches[1]}background:url({$image_url}){$matches[3]}";
            }
            else {
                $style = "{$matches[1]}background:url({$image_url}){$matches[3]}";
            }
