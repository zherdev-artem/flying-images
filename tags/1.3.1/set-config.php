...
function flying_images_set_default_config() {
    if (FLYING_IMAGES_VERSION !== get_option('FLYING_IMAGES_VERSION')) {
        ...
        if (get_option('flying_images_exclude_keywords') === false)
            update_option('flying_images_exclude_keywords', ['data:image','logo.png']);
        if (get_option('flying_images_responsive_images') === false)
            update_option('flying_images_responsive_images', false);
