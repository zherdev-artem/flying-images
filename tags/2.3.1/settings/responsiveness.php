...
function flying_pages_settings_responsiveness()
{
    ...
    if (isset($_POST['submit'])) {
        ...
        update_option('flying_images_enable_container_responsive_images', sanitize_text_field($_POST['enable_container_responsive_images']));
    }
    ...
    <form method="POST">
        ...
        <tbody>
            ...
            <tr>
                <th scope="row"><label>Restrict to container width<br/>(experimental)</label></th>
                <td>
                    <input name="enable_container_responsive_images" type="checkbox" value="1" <?php if ($enable_container_responsive_images) {echo "checked";} ?>>
                    <p class="description">If the image has a width attribute, restrict the max image width to the same. Otherwise restricts to max width of device.</p>
                </td>
            </tr>
        </tbody>
        </table>
